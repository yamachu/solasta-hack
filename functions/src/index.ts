...
export const myHttpHandler = https.onRequest(
  async (req: Request, res: Response) => {
    ...
    switch (actions.action) {
      case Command.ShowTemp: {
        ...
        res.send(
          `id: ${aircon.id} => ${aircon.attributes.name}, 電源: ${
            aircon.attributes.is_power
          }, 設定: ${aircon.attributes.setting}℃, 室温: ${
            aircon.attributes.temperature
          }℃`
        );
        return;
      }
      case Command.List: {
        ...
        res.send(
          status
            .map(
              v =>
                `id: ${v.id} => ${v.attributes.name}, 電源: ${
                  v.attributes.is_power
                }, 設定: ${v.attributes.setting}℃, 室温: ${
                  v.attributes.temperature
                }℃`
            )
            .join("\n")
        );
